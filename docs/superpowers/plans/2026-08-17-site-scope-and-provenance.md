...
> **Записано после исполнения:** раздел 2.1 покрыт **наполовину**, и это надо
> было сказать здесь с самого начала. Задача 7 сделала запрет резать ниже
> датума, строку `the clip holds the site` и доктрину в доках; поверх неё
> волна правок добавила вторую половину того же решения — `REGISTER_FLOOR` и
> `MESH_FLOOR` как заявление о том, какая из масс на участке есть нарисованное
> здание. Не сделано ничего из механики самой границы: нет `DECLARED_SITE`,
> нет `site.by` в `derived.json`, нет клипа по границе участка (`flatmap.parcel`
> не вызывается ниоткуда, кроме закомментированной строки скелета), нет
> отсечения соседей участком со счётом отброшенного.
>
> Следствие: `the clip holds the site` спрашивает только «дотянулся ли клип до
> постройки». Здание, не построившее свою территорию, получает зелёную строку
> на клипе, режущем участок пополам. Подробнее — в §2.1 спеки, подраздел «Что
> из этого построено, а что нет».

| Раздел спеки | Задача |
|---|---|
| 2.1 Клип по участку | 7 — наполовину, см. врезку выше |
